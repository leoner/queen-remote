...
ClientWorkforce.prototype.populateHandler = function(providerIds){
    ...
    providers.forEach(function(provider) {
        ...
        if (newProviders[name]) {
            newProviders[name] = pickProvider(newProviders[name], provider);
        } else {
            newProviders[name] = provider;
        }
    });

    providers = Object.keys(newProviders).map(function(name) {
        return newProviders[name]; 
    });
...
// 挑选压力最小的 Provider
function pickProvider(p1, p2) {
    return p1.getPendingWorkersSize() > p2.getPendingWorkersSize() ? p2 : p1;
}
